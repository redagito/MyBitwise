...
// Internal API
#define buffer_header_internal(buf) ((buffer_header_t*)((char*)(buf) - offsetof(buffer_header_t, buffer)))
...
// Free buffer memory
#define buffer_free(buf) ((buf) ? (free(buffer_header_internal(buf)), (buf) = NULL) : 0)
// Grow buffer to fit total n amount of elements
#define buffer_fit(buf, n) ((n) <= buffer_capacity(buf) ? 0 : ((buf) = buffer_grow_internal((buf), (n), sizeof(*(buf)))))
// Push new element into buffer
#define buffer_push(buf, ...) (buffer_fit((buf), 1 + buffer_length(buf)), (buf)[buffer_header_internal(buf)->length++] = (__VA_ARGS__))
// Put formatted string into the buffer
#define buffer_printf(buf, ...) ((buf) = buffer_printf_internal((buf), __VA_ARGS__))
...
* with element_size.
*/
void* buffer_grow_internal(const void* buffer, size_t new_length, size_t element_size);
